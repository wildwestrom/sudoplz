...
- **Less shell-config glue.** Sudo sanitizes `PATH`, which breaks shebang resolution in upstream's wrapper. This fork handles PATH and `SUDO_ASKPASS` identity inside `askpass` directly — install by pointing `SUDO_ASKPASS` at `./askpass`, nothing else.
...
4. Point `SUDO_ASKPASS` at the script in your shell config:
   ```bash
...
Configure these in `~/.config/secure-askpass/config.json` or the repo-local `askpass-config.json`. The shipped defaults are reasonable for a personal workstation.
...
1. Start ssh-agent if it isn't running
2. Check whether the key is loaded
3. Prompt for the passphrase via GUI if it isn't
4. Load the key into ssh-agent for the session

You enter the passphrase once per session. After that, sudo commands only need the confirmation dialog.
